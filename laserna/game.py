...
RIGHT = 'right'
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, RESET_SURF, RESET_RECT,SOLVE_SURF, SOLVE_RECT,NEXT_RECT,NEXT_SURF,A_SURF,A_RECT,TILECOLOR,clickable
    ...
    A_SURF, A_RECT = makeText('Solve as A Star',    TEXT, BGCOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 130)
                ...
                if (spotx, spoty) == (None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos):
                        ...
                        if k==0:
                            ...
                            allMoves = []
                        goal_state = [1, 2, 3, 4, 5, 6, 7, 8, 0]
                        A_SURF, A_RECT = makeText('NEXT',    TEXT, BGCOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 130)
                        solution, cost, explored_states = solve_8_puzzle_astar(state, goal_state)
                        print("Solution:")
                        ...
                        for action in solution:
                            soln.append(action);
                        file2 = open("puzzle_out_A_star.txt", "w")
                        file2.writelines(solution) #writing the solution TO PUZZLE.OUT
                        file2.close();
                        print("Please check the folder for solution/answer at puzzle_out")
                        temp = soln[k];
...
class PuzzleNode_A:
    def __init__(self, state, parent, action, cost, heuristic):
        """
        Initialize a PuzzleNode_A representing a state in the puzzle.

        Args:
            state (list): The current state of the puzzle.
            parent (PuzzleNode_A): The parent node in the search tree.
            action (str): The action taken to reach this state (e.g., 'Up', 'Down', 'Left', 'Right').
            cost (int): The cost to reach this state from the initial state.
            heuristic (int): The heuristic value estimated from this state to the goal state.
        """
        self.state = state
        self.parent = parent
        self.action = action
    ...
    def __lt__(self, other):
        """
        Compare two PuzzleNode_As based on their estimated total cost (f-cost) for A* search.

        Args:
            other (PuzzleNode_A): The other PuzzleNode_A to compare.

        Returns:
            bool: True if this node has a lower estimated total cost, False otherwise.
        """
...
def solve_8_puzzle_astar(initial_state, goal_state):
    ...
    Returns:
        tuple: A tuple containing the following elements:
            ...
            - int: The path cost of the solution.
            - int: The total number of states explored during the search.
    """
    ...
    def astar_search():
        ...
        closed_set = set()
        states_explored = 0
        start_node = PuzzleNode_A(initial_state, None, None, 0, heuristic(initial_state))
        ...
        while open_set:
            ...
            if is_goal_state(node.state):
                return get_solution_path(node), node.cost, states_explored
            ...
            for move, action in get_possible_moves(node.state):
                if tuple(move) not in closed_set:
                    cost = node.cost + 1
                    child_node = PuzzleNode_A(move, node, action, cost, heuristic(move))
    ...
    solution_path, path_cost, explored_states = astar_search()
    return solution_path, path_cost, explored_states
# ---------------------------------FOR EXER 2---------------------------------
